...
class ModelNetTrainer(object):
    ...
    def __init__(self, model, train_loader, val_loader, optimizer, loss_fn, \
                 model_name, log_dir, num_views=12):                   #这里从train_mvcnn那边传过来的model好像是cnet(SVCNN)
    ...
    def train(self, n_epochs):
        ...
        for epoch in range(n_epochs):
            ...
            ###origin & remote host 23.12.1
            # self.writer.add_scalar('params/lr', lr, epoch)  #原本的 先改了先吧。
            #local host
            ...
            for i, data in enumerate(self.train_loader):
                ...
                if self.model_name == 'mvcnn':
                    ...
                    #23.12.2传入nerf features
                    nerf_feature = Variable(data[2]).cuda()
                    ####
                ...
                # for item in target:
                #     print("target is",item)  #target is tensor(2, device='cuda:0') 这里的2有 从0~3 对应4个classes
                #     print("target's shape is ", item.shape) #

                self.optimizer.zero_grad()

                #23.12.2原来的，下面添加多一个nerf features,是给trainset用的，valset用不到就用else里面的
                # out_data = self.model(in_data)
                if self.model_name == 'mvcnn':
                    out_data = self.model(in_data,nerf_feature)
                else:
                    out_data = self.model(in_data)
                ######23.12.2
                ...
                #remote host & origin 23.12.1
                # self.writer.add_scalar('train/train_loss', loss, i_acc+i+1) #这个也改一下？？
                #local host 23.12.1
                ...
                # Print statements for debugging #23.12.10
                ...
                #remote host & origin 23.12.1
                # self.writer.add_scalar('train/train_overall_acc', acc, i_acc+i+1) #改一下左斜杠
                #local host 23.12.1
            ...
            if (epoch+1)%1==0:
                ...
                # self.writer.add_scalar('val/val_loss', loss, epoch+1)   #这边三个也改一下左斜杠
                #local host 23.12.1
                self.writer.add_scalar('val\\val_mean_class_acc', val_mean_class_acc, epoch + 1)
                self.writer.add_scalar('val\\val_overall_acc', val_overall_acc, epoch + 1)
        ...
        #23.12.1 origin & remote host
        # self.writer.export_scalars_to_json(self.log_dir+"/all_scalars.json") #这个也改一下
        #23.12.1 local host used
    ...
    def update_validation_accuracy(self, epoch):
        ...
        #23.11.16 原本的类目统计 40个class
        # wrong_class = np.zeros(40)
        # samples_class = np.zeros(40)

        #23.11.16 现在就弄了4个class
        ...
        for _, data in enumerate(self.val_loader, 0):
            ...
            if self.model_name == 'mvcnn':
                ...
                #23.12.7 为了解决报错说参数不匹配的问题
                nerf_feature = Variable(data[2]).cuda()  # NeRF features
                out_data = self.model(in_data, nerf_feature)  # Pass both to the model
                ####
